    ...
    try:
        ...
        try:
            macro_data = macro_col.collect(
                target={"KR": target["kr_date"], "US": target["us_date"]}
            )
        except Exception as e:
        ...
        try:
            theme_scan = scan_theme_strength(
                use_mock=use_mock,
                target={"KR": target["kr_date"], "US": target["us_date"]},
            )
        except Exception as e:
        ...
        try:
            if report_type == "morning":
                ...
                report_content = builder.build_morning_report(
                    ...
                    max_tokens=cfg.report.morning.get("max_tokens", 20000),
                )
            else:
                ...
                report_content = builder.build_evening_report(
                    ...
                    themes=cfg.themes.themes,
                    factor_accuracy=factor_accuracy,
                    max_tokens=cfg.report.evening.get("max_tokens", 20000),
                )
            print("  리포트 생성 완료")
        ...
        if send_email:
            ...
            if sender.is_configured():
                try:
                    ...
                    chart_images = generate_report_charts(
                        rating_dicts, changes, price_data,
                        target={"KR": target["kr_date"], "US": target["us_date"]},
                    )
                    if chart_images:
